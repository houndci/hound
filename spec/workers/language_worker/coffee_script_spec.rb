  ...
  describe CoffeeScript do
    describe "#run" do
      it "sends violations to hound" do
        ...
        worker = LanguageWorker::CoffeeScript.new(build_worker, commit_file, repo_config, pull_request)
    ...
    def commit_file(options = {})
      file = double("File", options.reverse_merge(patch: "", filename: "test.coffee"))
      commit = double(
        :commit,
        repo_name: "test/test",
        sha: "abc",
